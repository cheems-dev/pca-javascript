...
function zoomOutPlot({ centeredData, t, palette, zoomFrames = 50 }) {
...
export default function calcCovMatStep({
  ...
  if (t > zoomFrames + 50) {
    ...
    stroke(palette[palette.length - 3])
    text(
      `Cov(x, x) = Var(x) = ${covarianceMatrix[0][0].toFixed(2)}`,
      dataTableX + textSize() * 9,
      dataTableY + textSize() * 4
    )
  ...
  if (t > zoomFrames + 150) {
    ...
    stroke(palette[palette.length - 6])
    text(
      `Cov(y, y) = Var(y) = ${covarianceMatrix[1][1].toFixed(2)}`,
      dataTableX + textSize() * 9,
      dataTableY + textSize() * 5.2
    )
  ...
  if (t > zoomFrames + 50) {
    // covariance mat
    fill(255, 100)
    displayUtils.displayTable2d(displayCovMat, 30, dataTableY, [
      palette[palette.length - 3],
      palette[palette.length - 6],
    ])
  }
  ...
  if (t > zoomFrames + 225) {
    ...
    stroke(0)
    text(
      `Cov(x, y) = Cov(y, x) = ${covarianceMatrix[0][1].toFixed(2)}`,
      dataTableX + textSize() * 9,
      dataTableY + textSize() * 6.3
    )
    ...
    fill(255, 0)
    displayUtils.displayTable2d(displayCovMat, 30, dataTableY, [0, 0])
  }
